...
block.rmse.df <- na.omit(block.rmse.df)


mse.compare.df<-block.mse.df
mape.compare.df<-block.mape.df
rmse.compare.df <-block.rmse.df
...
for(wsize in wsize.list){
  ...
  rmse.compare.df <- merge(x=rmse.compare.df,y=lm.rmse.df,by="cutoff")
...
write.csv(mape.compare.df,file.path(output.folder,paste("mape_block_lm_compare_numtrees=",toString(numtrees),".csv",sep="")),row.names=FALSE)
write.csv(rmse.compare.df,file.path(output.folder,paste("rmse_block_lm_compare_numtrees=",toString(numtrees),".csv",sep="")),row.names=FALSE)
