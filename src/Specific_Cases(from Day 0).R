...
for (windowsize in c(15,7,3,1)){
  ...
  for(cutoff in cutofflist){
    ...
    if (cutoff + predictionsize <= latest_date){
      ...
      lm.rmse.list <- c(lm.rmse.list, mean(restricted_state_df2$lm.rmse))
      slm.rmse.list <- c(slm.rmse.list, mean(restricted_state_df2$slm.rmse))
      ...
      print(paste("cutoff=",toString(cutoff)," slm.mse=", toString(mean(restricted_state_df2$slm.mse)),
                  ...
                  " lm.mape=",toString(mean(restricted_state_df2$lm.mape)), " lm.rmse=", 
                  toString(mean(restricted_state_df2$lm.rmse))," slm.rmse",toString(mean(restricted_state_df2$slm.rmse)),sep=""))
    ...
    if (windowsize==1){write.csv(restricted_state_df2,backtest_file_path,row.names=FALSE)}
    # temp measure
